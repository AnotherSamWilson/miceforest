...
var_mmc = {"sepal width (cm)": 5, "petal width (cm)": 0}
...
def test_csmice():
    ...
    assert all(compdat.isna().sum()[["petal width (cm)", "sepal width (cm)"]] == [0, 0])
...
def test_csimpute_new():
    ...
    assert all(
        newdatcomp.isna().sum()[["petal width (cm)", "sepal width (cm)"]] == [0, 0]
    )
