...
from .utils import _distinct_from_list_fast, _copy_and_remove, _setequal, _list_union
...
class _ImputationSchema:
    ...
    def __init__(
        ...
        # address issue #4:
        self.predictor_vars = _distinct_from_list_fast(
            [x for _, value in variable_schema.items() for x in value]
            )
        self.response_vars = list(variable_schema)
        self.all_vars = _distinct_from_list_fast(self.response_vars + self.predictor_vars)
